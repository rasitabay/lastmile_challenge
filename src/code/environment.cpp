...
bool TEnvironment::terminationCondition(){
	//4.32 is selected to ensure that even when 3050 routes are found at max time, there still remains 15 minutes.
	//cout << (double)(clock() - this->fTimeStart)/(double)CLOCKS_PER_SEC;
	if ((double)(clock() - this->fTimeStart)/(double)CLOCKS_PER_SEC > 4.32) return true;
